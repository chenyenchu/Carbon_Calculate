...
<script lang="ts">
    ...
    function carbon_emission(carbon){
        fakecarbon = carbon - 0.173 * train_km
        return fakecarbon
    }
    ...
    function getCarbon() {
        if (time_input){
            if (minute !== undefined) {
                if (minute !== null) {
                    train_km =  (minute / 60) * 40;
                    carbon = train_km * (21 / 1000);
                }
                else {
                    alert(`資料沒輸入完整啦`);
                    minute = undefined;
                    time_input = false;
                    console.log("One or both of the station of train_start and train_end is NULL");
                }
            }
            else {
                alert(`資料沒輸入完整啦`);
                minute = undefined;
                time_input = false;
                console.log("One or both of the station of train_start and train_end is undefined")
            }
            return carbon;
        }
    }
    ...
    const addtrain = async () => {
        if (user) {
            ...
            train_start = train_end = minute = undefined;
            time_input = false;
        }
    };
...
<!-- Page Header Start -->
<div class="container-fluid page-header py-5 mb-5 wow fadeIn" data-wow-delay="0.1s" style="background: linear-gradient(rgba(15, 66, 41, .6), rgba(15, 66, 41, .6)), url('/img/train_bg.jpg') center center no-repeat;
background-size: cover;">
    <div class="container text-center py-5">
        <h1 class="display-3 text-white mb-4 animated slideInDown">火車</h1>
        <nav aria-label="breadcrumb animated slideInDown">
            <ol class="breadcrumb justify-content-center mb-0">
                <li class="breadcrumb-item"><a href="#">大眾運輸</a></li>
                <li class="breadcrumb-item"><a href="#">火車</a></li>
...
<div class="container-xxl py-5">
    <div class="container">
        <div class="text-center mx-auto wow fadeInUp" data-wow-delay="0.1s" style="max-width: 500px;">
            <p class="fs-5 fw-bold text-primary">火車計算</p>
            <h1 class="display-5 mb-5">火車坐多久?</h1>
        </div>
...
<!--        </div>-->
     ...
     --            &lt;!&ndash;            紀錄表格開始&ndash;&gt;
                  ...
                  &lt;!&ndash;            輸入式的表單填寫&ndash;&gt;
                  <div class="mb-3 mt-3 col">
                      <label class="form-label">起始站</label>


                      <input class="form-control" type="text" placeholder="輸入起始站" bind:value={bus_start}/>
                  </div>
                  <div class="mb-3 mt-3 col">
                      <label class="form-label">終點站</label>

                      <input class="form-control" type="text" placeholder="輸入終點站" bind:value={bus_end}/>
                  </div>

            </div>-->
        <div class="container p-5 my-5 border row">
            <h1>輸入乘車時間(分鐘)</h1>
            <p>請輸入你今天搭乘火車的時間(分鐘)</p>
            <!--                        輸入式的表單填寫-->
            <label class="form-label">乘車時間(分鐘)</label>
            <input class="form-control" type="number" min="0" placeholder="輸入火車乘車時間" bind:value={minute}/>
            ...
            <div class="container p-5 my-5 border ">
                ...
                {#if (user)}
                    <div style="text-align:right">
                        <!--               按下去會紀錄資料到firebase-->
                        <button type="button" class="btn btn-secondary" on:click={addtrain}>儲存</button>
                    </div>
                {/if}
            </div>
        {/if}
                    ...
                    <div class="bg-light rounded p-4 p-sm-5 wow fadeInUp" data-wow-delay="0.1s" id="food-records">
                        <div class="sm:p-7 p-4">
                            <table class="w-full text-left">
                                ...
                                {#each todos.filter(todos => todos.type === "train" && todos.userId === id) as records, i}
                                    <tr>
                                        ...
                                        <td class="sm:p-3 py-2 px-3 border-b border-gray-200 dark:border-gray-800">
                                            <div class="flex items-center">
                                                <div class="sm:flex hidden flex-col">
                                                    {Math.round(records.d_emission*100)/100}
                                                </div>
                                            </div>
                                        </td>
                                        <td>
